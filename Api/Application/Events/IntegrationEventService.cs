	...
	public class IntegrationEventService : IIntegrationEventService
	{
			...
			where TEvent : IntegrationEvent
		{
			...
			var policy = Policy
				...
				{
					_logger.LogWarning(exception, "Could not publish event #{EventId} after {Timeout} seconds: {ExceptionMessage}.", @event.IntegrationEventId, $"{timeSpan.TotalSeconds:n1}", exception.Message);
				});
			...
			using (var channel = _persistentConnection.CreateModel())
			{
				...
				policy.Execute(() =>
				{
					...
					_logger.LogTrace("Publishing event to RabbitMQ with ID #{EventId}...", @event.IntegrationEventId);
					...
					_logger.LogTrace("Published event with ID #{EventId}.", @event.IntegrationEventId);
				});
			}
		}
